...
class Package:
    ...
    def get_costs(self):  # vytvoření metody get_costs() pro běžný balík
        cena_prepravy = 0
        if self.weight <= 2:
            cena_prepravy = 150
...
class ValuablePackage(Package): 
    ...
    def get_costs(self): # vytvoření metody get_costs() pro cenný balík
...
class ValuablePackage(Package):
    def __init__(self, address, weight, state, value):
        super().__init__(self, address, weight, state)
        self.value = value
